...
def plot_network3D_2(nodes, elements, fixed_nodes, external_loads):
    ...
    for node_id, (x, y, z) in nodes.items():
        if node_id in fixed_nodes:
            ax.scatter(x, y, z, color='blue', s=50, label='Fixed' if node_id == fixed_nodes[0] else "")
        elif any(val != 0.0 for val in external_loads[node_id]):
            ax.scatter(x, y, z, color='red', s=50, label='Loaded' if node_id == list(external_loads.keys())[0] else "")
        else:
            ax.scatter(x, y, z, color='black', s=25, label='Normal' if node_id == 1 else "")
